...
async function generateImagePairs() {
    ...
    for(let i =0;i<cards.length;i++) {
        ...
        // Verificar se já passou dos cards disponíveis
        if(!imagePairs[cards[i]]) {
            const id  = Math.floor(Math.random()*1000) +1
            const url =`https://picsum.photos/id/${id}/300/400`;

            //Preencher o imagePairs com o índice dos cards 
...
async function createCards() {
    // Esperar os pares da imagem chegar 
...
function checkForMatch() {
    ...
    // Lógica para desabilitar as cartas e se não for vamos virar as cartas 
